package ch.epfl.vlsc.cpp.phase;
...
public class CppBackendPhase implements Phase {
     ...
     * @param backend
     */
    private void generateAuxiliary(CppBackend backend) {
    ...
    private void generateInstances(CppBackend backend) {
        for (Instance instance : backend.task().getNetwork().getInstances()) {
            GlobalEntityDecl entityDecl = backend.globalnames().entityDecl(instance.getEntityName(), true);
    ...
    private void copyResources(CppBackend backend) {
        try {
            // -- Copy Runtime
    ...
    private void generateGlobals(CppBackend backend){
        backend.globals().generateHeader();
    }

    private void generateMain(CppBackend backend){
        backend.main().generateMain();
    }

    private void generateCMakeLists(CppBackend backend){
    ...
    public CompilationTask execute(CompilationTask task, Context context) throws CompilationException {
        ...
        CppBackend backend = MultiJ.from(CppBackend.class)
                .bind("task").to(task)
