...
public interface Allocate {
    ...
    default void apply(ListType type, String pointer) {
        emitter().emit("%s = (%s %s) calloc(%s, sizeof(%s));", pointer, backend().typeseval().type(type), backend().declarations().getPointerDims(type), type.getSize().getAsInt(), backend().typeseval().pointerType(type));

        if (type.getElementType() instanceof ListType) {
            ListType lt = (ListType) type.getElementType();
            allocate(lt, pointer, lt.getSize().getAsInt(), "");
        }
    }

    default void allocate(ListType type, String pointer, int size,  String index) {
        ...
        if (type.getElementType() instanceof ListType) {
            ListType lt = (ListType) type.getElementType();
            allocate(lt, pointer, lt.getSize().getAsInt(), dim);
        }
