...
ART_ACTION_SCHEDULER(art_Sink_16bit_txt_action_scheduler) {
    ...
    numTokens = pinAvailIn_int16_t(ART_INPUT(0));
    ART_ACTION_SCHEDULER_LOOP {
        ...
        if (numTokens > 0) {
            ...
            int32_t token = pinRead_int16_t(ART_INPUT(0));
...
static const ActionDescription actionDescriptions[] = {
        {"actionAtLine_7", "actionAtLine_7", portRate_1, 0}
};
...
#ifdef CAL_RT_CALVIN
ActorClass ActorClass_art_Sink_16bit_txt = INIT_ActorClass(
        (char*) "ART.art_Sink_16bit_txt",
        ActorInstance_art_Sink,
        constructor,
        setParam,
        art_Sink_16bit_txt_action_scheduler,
        destructor,
