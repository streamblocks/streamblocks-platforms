...
public interface BranchingController {
    ...
    default void emitController(String name, ActorMachine actorMachine) {
        ...
        emitter().emit("\treturn RETURN_IDLE;");
        emitter().decreaseIndentation();
        emitter().emit("}");
        emitter().emitNewLine();
    ...
    default void emitInstruction(ActorMachine am, String name, Wait wait, Map<State, Integer> stateNumbers, Function<Instruction, BitSet> initialize, Instruction previous) {
        ...
        if (previous instanceof Test) {
            Test test = (Test) previous;
            if (am.getCondition(test.condition()) instanceof PortCondition) {
                PortCondition condition = (PortCondition) am.getCondition(test.condition());
                if (condition.isInputCondition()) {
                    waitKind = String.format("%s", "RETURN_WAIT_INPUT");
                } else {
                    waitKind = String.format("%s", "RETURN_WAIT_OUTPUT");
                }
                String portName = ((PortCondition) am.getCondition(test.condition())).getPortName().getName();
            } else {
                waitKind = String.format("%s", "RETURN_WAIT_PREDICATE");
            }
        }

        emitter().emit("this->program_counter = %d;", stateNumbers.get(wait.target()));
        //emitter().emit("goto out;");
        emitter().emit("return %s;", waitKind);
